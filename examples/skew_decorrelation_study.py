...
CANCEL = (len(sys.argv) > 4 and sys.argv[4] == 'cancel')
...
for r in range(R_avg):
    ...
    for b in range(2):                        # 0: I-dither record, 1: Q-dither
        if not SHARED:
            xI, xQ = make_data()              # independent records in time
        ...
        if CANCEL:                      # TX-side perfect pre-cancellation
            y0I = np.fft.irfft(np.fft.rfft(xI) * HII, N)
            y0Q = np.fft.irfft(np.fft.rfft(xQ) * HQQ, N)
            p = p - (y0I * y0I + y0Q * y0Q)
        v = W @ (p - p.mean())
