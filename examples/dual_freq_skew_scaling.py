...
Nd = N // D_adc
bin_hz = fs / N
m1, m2 = 52, 60                # nu1 = 39.67 MHz, nu2 = 45.78 MHz
...
for r in range(R_avg):
    ...
    for typ, (cI, cQ) in enumerate(((c1, c2), (c2, c1))):
        dyI = np.fft.irfft(np.fft.rfft(eps * xI * cI) * HII, N)
        dyQ = np.fft.irfft(np.fft.rfft(eps * xQ * cQ) * HQQ, N)
        p_diff = 2 * (ydI * dyI + ydQ * dyQ) + dyI**2 + dyQ**2
        v = W @ p_diff
...
sk_t = 0.5 * (true_skew(m1) + true_skew(m2))

def skew_from(sl):
    B_I1 = lines[0, 0, sl].mean()   # I @ nu1 (type 1)
    B_Q1 = lines[0, 1, sl].mean()   # Q @ nu2 (type 1)
    B_Q2 = lines[1, 0, sl].mean()   # Q @ nu1 (type 2)
    B_I2 = lines[1, 1, sl].mean()   # I @ nu2 (type 2)
    d_a = -np.angle(B_Q1 * np.conj(B_I2)) / (2 * np.pi * nu2)  # argM(nu2) cancels
    d_b = -np.angle(B_Q2 * np.conj(B_I1)) / (2 * np.pi * nu1)  # argM(nu1) cancels
    return 0.5 * (d_a + d_b)

np.savez("dual_freq_lines.npz", lines=lines, nu1=nu1, nu2=nu2, sk_t=sk_t)
