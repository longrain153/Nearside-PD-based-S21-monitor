...
for r in range(R_avg):
    ...
    for k in range(K):
        for b, (Xbr, Hbr, yd) in enumerate(((XI, HII, ydI), (XQ, HQQ, ydQ))):
            ...
            # NO data-aided cancellation: monitor projects the full record
            p_full = (yd + dy) ** 2 + (ydQ if b == 0 else ydI) ** 2
            v = W @ (p_full - p_full.mean())
            v += E_noise @ (noise_std * rng.standard_normal(Nd))
            acc[2 * k + b] += v
    if r % 100 == 0:
        print(f"[sweep] r={r} elapsed {time.time()-t_start:.0f}s", flush=True)
spec = acc / R_avg
...
fig.suptitle("Minimal scheme sweep: shallow dither, NO data cancellation, async ADC\n"
             "common quadratic 6 rad + 3 ps skew, "
             f"$\\nu$={nu/1e6:.0f} MHz, $\\varphi$=0, live 16QAM, "
             f"{R_avg*N/fs*1e3:.1f} ms/point")
fig.tight_layout()
fig.savefig("simple_sweep.png", dpi=150)
np.savez("simple_sweep.npz", B_I=B_I, B_Q=B_Q, tau_I=tau_I, tau_Q=tau_Q,
